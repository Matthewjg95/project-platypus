...
#include <ESPping.h>   // Library: "espping" (GitHub: 4-20ma/espping)
#include <cstring>
#include <cmath>
...
static const uint8_t PI4_REG_OUT  = 0x01;
static const uint8_t PI4_REG_CFG  = 0x03;
...
// ═══════════════════════════════════════════════════════════════════════════
void loop() {
    // Note: esp_task_wdt_reset() not needed on Arduino-ESP32 3.x (loop task not subscribed)
...
uint8_t expRead(uint8_t reg){
    ...
    Wire.requestFrom((uint8_t)IO_EXP_ADDR,(uint8_t)1);
    return Wire.available() ? Wire.read() : 0xFF;
}
void initExpander(){
    uint8_t cfg=expRead(PI4_REG_CFG); cfg&=~(1u<<IO_EXP_PIN);
    expWrite(PI4_REG_CFG,cfg);
...
void drawMenuScreen(){
    ...
    drawBtnBar("BACK",C_INACT,"",C_BG,"",C_BG,"SETTINGS",C_GOLD);
}
...
// ═══════════════════════════════════════════════════════════════════════════
void startLog(){
    g_logFile=SD.open(LOG_FILE,FILE_APPEND);
