...
public:
    ...
    // Mark every slot stale. Call at scan start so a new scan can never
    // sample a leftover frame from the previous one.
    void invalidateAll() {
        for (int i = 0; i < FB_SLOTS; ++i) _slots[i].valid = false;
    }
