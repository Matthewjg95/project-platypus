...
bool UartReceiver::begin(HardwareSerial& serial, uint32_t baud, int rxPin, int txPin) {
    ...
    _serial = &serial;
    // Must hold MULTIPLE full JPEG frames: real-room frames run ~14 KB, and a
    // slow render pass during SCAN can stall draining for 100+ ms. NOTE:
    // setRxBufferSize is a NO-OP after begin() on arduino-esp32 — the old
    // post-begin 4096 call never applied, so the driver ran on its tiny
    // default and overflowed -> CRC storm that killed every frame mid-scan.
    _serial->setRxBufferSize(32768);
